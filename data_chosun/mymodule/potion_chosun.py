    ...
    try:
        ...
        if result_maul == True:
            ...
            while buy is False:
                ...
                full_path = "c:\\my_games\\chosun\\data_chosun\\imgs\\potion\\jadong_bogwan_btn.PNG"
                img_array = np.fromfile(full_path, np.uint8)
                img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
                imgs_ = imgs_set_(640, 740, 770, 780, cla, img, 0.85)
                if imgs_ is not None and imgs_ != False:
                    print("jadong_bogwan_btn", imgs_)
                    buy = True
                    ...
                    full_path = "c:\\my_games\\chosun\\data_chosun\\imgs\\potion\\out_chango_btn.PNG"
                    img_array = np.fromfile(full_path, np.uint8)
                    img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
                    imgs_ = imgs_set_(210, 340, 700, 700, cla, img, 0.85)
                    if imgs_ is not None and imgs_ != False:
                        print("out_chango_btn", imgs_)
                        click_pos_reg(imgs_.x, imgs_.y, cla)
                    else:
                        for i in range(1):
                            full_path = "c:\\my_games\\chosun\\data_chosun\\imgs\\potion\\chango_point_" + str(i) + ".PNG"
                            img_array = np.fromfile(full_path, np.uint8)
                            img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
                            imgs_ = imgs_set_(0, 270, 920, 800, cla, img, 0.85)
                            if imgs_ is not None and imgs_ != False:
                                print("potion_point_", str(i), imgs_)
                                click_pos_reg(imgs_.x, imgs_.y + 70, cla)
                            time.sleep(1)
        ...
        while maul_ is False:
            ...
            if imgs_ is not None and imgs_ != False:
                ...
                if imgs_ is not None and imgs_ != False:
                    ...
                    full_path = "c:\\my_games\\chosun\\data_chosun\\imgs\\potion\\chango_btn.PNG"
                    img_array = np.fromfile(full_path, np.uint8)
                    img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
                    imgs_ = imgs_set_(650, 380, 850, 750, cla, img, 0.8)
                    if imgs_ is not None and imgs_ != False:
                        ...
                        for i in range(10):
                            ...
                            if result_loading == True:
                                ...
                                result_out = out_check(cla, "chango")
                                if result_out == True:
                                    maul_ = True
                                    buy_ready = True
                                    break
                ...
                result_out = out_check(cla, "chango_ready")
                if result_out == True:
                    click_pos_2(800, 385, cla)
                else:
                    clean_screen(cla)
